	...
	if (strcasecmp(password_connect, password_receive))
	{
		if (!password_receive_authmethod)
		{
			...
			config_warn("Link block '%s' has a different connect/receive password. "
			            "This is no longer supported in DevCenterIRCd 1.0",
			            ce->ce_vardata);
...
void update_conf(void)
{
	...
	{
		config_status("");
		config_status("%d configuration file(s) upgraded. You can now boot DevCenterIRCd with your freshly converted conf's!", upgraded_files);
