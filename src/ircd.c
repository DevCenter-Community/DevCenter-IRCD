...
VOIDSIG s_die()
{
	...
	if (!IsService)
	{
		...
		SC_HANDLE hService = OpenService(hSCManager, "DevCenterIRCd", SERVICE_STOP);
...
static int bad_command(const char *argv0)
{
	...
	(void)printf
	    ("Usage: %s [-f <config>] [-h <servername>] [-p <port>] [-x <loglevel>] [-t] [-F]\n"
	     ...
	     "                 DevCenterIRCd under gdb or when playing around with settings\n"
	...
	if (!IsService) {
		MessageBox(NULL,
		    "Usage: DevCenterIRCd [-h servername] [-p portnumber] [-x loglevel]\n",
		    "DevCenterIRCD/32", MB_OK);
	}
#endif
	return (-1);
}
...
static void do_version_check()
{
	...
	{
#ifndef _WIN32
		version_check_logerror("Header<->library mismatches can make DevCenterIRCd *CRASH*! "
		                ...
		                "be sure to recompile DevCenterIRCd.");
#else
		version_check_logerror("Header<->library mismatches can make DevCenterIRCd *CRASH*! "
...
#endif
{
	...
	if (euid == 0)
	{
		fprintf(stderr,
			"** ERROR **\n"
			"You attempted to run DevCenterIRCd as root. This is VERY DANGEROUS\n"
			"as any compromise of your DevCenterIRCd will result in full\n"
			"privileges to the attacker on the entire machine.\n"
			"You MUST start DevCenterIRCd as a different user!\n"
			"\n"
		  ...
		  case 'v':
			  if (!IsService) {
				  MessageBox(NULL, version,
				      "DevCenterIRCD/Win32 version", MB_OK);
			  }
#endif
			  exit(0);
		  case 'C':
...
# endif
	...
	uid_init();
	run_configuration();
	ircd_log(LOG_ERROR, "DevCenterIRCd started.");
	...
	if (TIMESYNCH)
	{
		if (!unreal_time_synch(TIMESYNCH_TIMEOUT))
			ircd_log(LOG_ERROR, "TIME SYNCH: Unable to synchronize time: %s. "
			                    "This means DevCenterIRCd was unable to synchronize the IRCd clock to a known good time source. "
