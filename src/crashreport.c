...
char *find_best_coredump(void)
{
	...
	*best_fname = '\0';
	...
	hFile = FindFirstFile("devcenterircd.*.core", &hData);
...
void crash_report_fix_libs(char *coredump, int *thirdpartymods)
{
	...
	snprintf(cmd, sizeof(cmd), "echo info sharedlibrary|gdb %s/devcenterircd %s 2>&1",
		BINDIR, coredump);
	...
	while((fgets(buf, sizeof(buf), fd)))
	{
		 ...
		 * <many spaces>    No        /home/blabla/devcenterircd/tmp/5114DF16.m_kick.so
		 */
		if (!strstr(buf, " No "))
			continue;
...
int crash_report_backtrace(FILE *reportfd, char *coredump)
{
	...
	snprintf(cmd, sizeof(cmd), "gdb -batch -x %s %s/devcenterircd %s 2>&1",
		buf, BINDIR, coredump);
...
void crash_report_header(FILE *reportfd, char *coredump)
{
	...
	fprintf(reportfd, "== DEVCENTERIRCD CRASH REPORT ==\n"
	                  "\n"
	                  "SYSTEM INFORMATION:\n");
	
	fprintf(reportfd, "DevCenterIRCd version: %s\n", VERSIONONLY);
...
int corefile_vs_binary_mismatch(char *coredump)
{
	...
	snprintf(fname, sizeof(fname), "%s/devcenterircd", BINDIR);
	...
	return 0; /* GOOD! */
#else
	return 0; /* guess we don't check this on Windows? Or will we check DevCenterIRCd.exe... hmm.. yeah maybe good idea */
#endif
}
...
int attach_coredump(FILE *fdo, char *coredump)
{
	...
	printf("Please wait...\n");
	snprintf(fname, sizeof(fname), "tar c %s/devcenterircd %s %s 2>/dev/null|(bzip2 || gzip) 2>/dev/null",
...
void report_crash(void)
{
	...
	{
	    printf("** IMPORTANT **\n"
               "Your DevCenterIRCd crashed and you have 3rd party modules loaded (modules created\n"
               "by someone other than the DevCenterIRCd team). If you installed new 3rd party\n"
               ...
               "the DevCenterIRCd team.\n"
               "The reason we ask you to do this is because more than 95%% of the crash issues\n"
               "reported nowadays are caused by 3rd party modules and not by an DevCenterIRCd bug.\n"
               "\n");
	}
		
	if (report_pref == REPORT_NEVER)
	{
		printf("Crash report will not be sent to DevCenterIRCd Team.\n\n");
	...
	if (report_pref == REPORT_ASK)
	{
		...
		printf("Shall I send a crash report to the DevCenterIRCd developers?\n");
		if (!thirdpartymods)
	...
	{
		printf("\nThe crash report has been sent to the DevCenterIRCd developers. "
		       "Thanks a lot for helping to make DevCenterIRCd a better product!\n\n");
	}

#else
	/* Windows */
	if (MessageBox(NULL, "DevCenterIRCd crashed. May I send a report about this to the DevCenterIRCd developers? This helps us a lot.",
	                     "DevCenterIRCd crash",
	                     MB_YESNO|MB_ICONQUESTION) == IDYES)
	{
		...
		if (crashreport_send(fname))
		{
			MessageBox(NULL, "The crash report has been sent to the DevCenterIRCd developers. "
			           ...
			           "DevCenterIRCd crash report sent", MB_ICONINFORMATION|MB_OK);
		}
	}
#endif
...
#ifdef _WIN32
	if (MessageBox(NULL, "Start DevCenterIRCd again?",
	                     "DevCenterIRCd crash",
	                     MB_YESNO|MB_ICONQUESTION) == IDYES)
	{
		StartUnrealAgain();
