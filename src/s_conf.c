...
void upgrade_conf_to_34(void)
{
	...
	else
	{
		config_error("We offer a configuration file converter to convert 3.2.x conf's to 4.0, however this "
		             "is not available when running as a service. If you want to use it, make DevCenterIRCd "
		             "run in GUI mode by running 'devcenter uninstall'. Then start DevCenterIRCd.exe and when "
		             "it prompts you to convert the configuration click 'Yes'. Check if DevCenterIRCd boots properly. "
		             "Once everything is looking good you can run 'devcenter install' to make DevCenterIRCd run "
		             "as a service again."); /* TODO: make this unnecessary :D */
	}
#else
	config_error("To upgrade it to the new 4.0 format, run: ./devcenterircd upgrade-conf");
#endif
...
#ifdef _WIN32
		if (!strcmp(filename, "conf/devcenterircd.conf"))
		{
			if (file_exists("devcenterircd.conf"))
			{
				config_error("Note that 'devcenterircd.conf' now belongs in the 'conf' subdirectory! (So move it to there)");
			} else {
				config_error("New to DevCenterIRCd? Be sure to read https://www.unrealircd.org/docs/Installing_%%28Windows%%29");
			}
		}
#endif
		return -1;
...
int	_test_oper(ConfigFile *conf, ConfigEntry *ce)
{
	...
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		...
		if (!cep->ce_entries)
		{
			...
			{
				config_error("%s:%i: oper::flags no longer exists. DevCenterIRCd 1 uses a new style oper block.",
					cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
				errors++;
				need_34_upgrade = 1;
			}
	...
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		...
		else if (!strcmp(cep->ce_varname, "new-linking-protocol"))
		{
			...
			for (cepp = cep->ce_entries; cepp; cepp = cepp->ce_next) {
				...
				{
					config_error("%s:%i: set::dns::%s no longer exist in DevCenterIRCd 1. "
			...
			config_error("%s:%i: set::hosts has been removed in DevCenterIRCd 1. You can use oper::vhost now.",
				cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
			errors++;
			need_34_upgrade = 1;
		}
...
int	_conf_loadmodule(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if (strstr(ce->ce_vardata, "modules/cloak") && !strcmp(conf->cf_filename, "modules.conf"))
	{
		...
		config_error("DevCenterIRCd 1 then please change the include \"modules.conf\";");
		config_error("into an include \"modules.default.conf\"; (probably in your");
		config_error("conf/devcenterircd.conf). Yeah, we changed the file name.");
...
void start_listeners(void)
{
	...
	*boundmsg_ipv4 = *boundmsg_ipv6 = '\0';
	...
	for (listenptr = conf_listen; listenptr; listenptr = listenptr->next)
	{
		...
		if (!(listenptr->options & LISTENER_BOUND) && !listenptr->flag.temporary)
		{
			if (add_listener2(listenptr) == -1)
			{
				...
				if (loop.ircd_booted)
				{
					ircd_log(LOG_ERROR, "DevCenterIRCd is now also listening on %s:%d (%s)%s",
	...
	if (!loop.ircd_booted)
	{
		...
		ircd_log(LOG_ERROR, "DevCenterIRCd is now listening on the following addresses/ports:");
...
int ssl_tests(void)
{
	if (have_ssl_listeners == 0)
	{
		...
		config_warn("Add this to your devcenterircd.conf: listen { ip %s; port 6697; options { ssl; }; };",
