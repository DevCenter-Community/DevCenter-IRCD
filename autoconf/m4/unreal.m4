...
[
  ...
  You have decided to build DevCenterIRCd with libcURL (remote includes) support.
  However, you have system-installed c-ares support has either been disabled
  (--without-system-cares) or is unavailable.
  Because DevCenterIRCd will use a bundled copy of c-ares which may be incompatible
  with the system-installed libcURL, this is a bad idea which may result in error
  messages looking like:

  	\`\`[error] devcenterircd.conf:9: include: error downloading '(http://example.net/ex.conf)': Could not resolve host: example.net (Successful completion)''

  Or DevCenterIRCd might even crash.

  Please build DevCenterIRCd with --with-system-cares when enabling --enable-libcurl
]])
		])
